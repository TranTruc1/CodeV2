...
function createButtons() {
    ...
    if (searchByRole) {
        // Lọc Tên Page trước (Nếu có gõ phần Tên Page ở cuối)
        if (pageKeywordLower) {
        ...
        // Tính toán trước trạng thái Xanh/Đỏ cho Tên Người để lát nữa Sắp Xếp
        ...
        // Lọc thường: Khớp bát kỳ từ khóa nào với ID hoặc Tên
        if (roleKeywordsLower.length > 0) {
            displayData = displayData.filter(item => {
                const nameLower = item.attribute.toLowerCase();
    ...
    // 2. SẮP XẾP DỮ LIỆU (TỔNG HỢP 2 ĐIỀU KIỆN)
    const sortType = document.getElementById('sortSelect')?.value || 'default';
    
    displayData.sort((a, b) => {
        // Ưu tiên 1: Đẩy màu Xanh lên đầu (Chỉ áp dụng khi đang bật tìm Quyền và có gõ tên)
        if (searchByRole && roleKeywordsLower.length > 0) {
            if (a._isFullMatch !== b._isFullMatch) {
                return a._isFullMatch ? -1 : 1; // True (-1) nằm trên False (1)
            }
        }
        
        // Ưu tiên 2: Sắp xếp theo lựa chọn ở Menu thả xuống (Like, Quyền...)
    ...
    displayData.forEach((item, index) => {
        ...
        // Bôi màu theo kết quả đã tính ở trên
        if (searchByRole && roleKeywordsLower.length > 0) {
...
function renderAllUsersModal() {
    ...
    // Sự kiện nút Tìm Kiếm người đã chọn trong Modal
    ...
    if (searchBtn) {
        ...
        newSearchBtn.addEventListener('click', () => {
            ...
            // Điền vào form, TỰ ĐỘNG THÊM PHẨY Ở CUỐI ĐỂ TÁCH BIỆT KHỎI PHẦN "TÊN PAGE"
...
// ========== CHỨC NĂNG RELOAD DATA TỰ ĐỘNG (GỌI TỪ TOKEN.JS) ==========
...
// ========== CHỨC NĂNG RELOAD DATA TỰ ĐỘNG (CÓ KHÓA TƯƠNG TÁC) ==========
...
async function ensureTabAtRequiredUrl(tabId, requiredUrl) {
    // Kiểm tra URL hiện tại, nếu không đúng thì cập nhật
    ...
    if (tab.url !== requiredUrl) {
        ...
        await sleep(3000); // Chờ trang load
    }
    return tab;
}
...
async function reloadDataFromFB() {
    ...
    // 2. Thử lấy token nhiều lần, kiểm tra URL mỗi lần
    let tokenResult = null;
    let attempts = 0;
    const maxAttempts = 12; // tăng lên 12 lần
    while (attempts < maxAttempts && !reloadCancelled) {
        // Đảm bảo tab vẫn đang ở đúng URL
        ...
        updateLoadingStatus(`Đang lấy token (lần ${attempts+1}/${maxAttempts})...`);
        ...
        await sleep(2000);
        if (attempts === 6) {
            updateLoadingStatus("Lâu quá, thử reload lại tab...");
    ...
    try {
        ...
        for (let item of formattedData) {
            if (!item.img3 && item.image) {
                item.img3 = await convertImageToBase64(item.image);
            }
        }
        currentPageData = formattedData;
        await saveAllData();
...
// Các hàm findTabByUrl, createTab vẫn giữ nguyên
async function findTabByUrl(url) {
    return new Promise((resolve) => {
        chrome.tabs.query({ url: url }, (tabs) => {
            resolve(tabs.length ? tabs[0] : null);
...
// Gắn sự kiện cho nút Reload Data
